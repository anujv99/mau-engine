...
namespace mau {
  ...
  void Input::KeyPressEventHandler(KeyPressEvent &event) {
    key_state[event.GetKey()] = true;
  }

  void Input::KeyReleaseEventHandler(KeyReleaseEvent &event) {
    key_state[event.GetKey()] = false;
  }

  void Input::MousePressEventHandler(MousePressEvent &event) {
    mouse_state[event.GetButton()] = true;
  }

  void Input::MouseReleaseEventHandler(MouseReleaseEvent &event) {
    mouse_state[event.GetButton()] = false;
  }

  void Input::MouseMoveEventHandler(MouseMoveEvent &event) {
    mouse_pos = {event.GetX(), event.GetY()};
  }

  void Input::MouseScrollEventHandler(MouseScrollEvent &event) {
    mouse_scroll = {event.GetX(), event.GetY()};
  }
