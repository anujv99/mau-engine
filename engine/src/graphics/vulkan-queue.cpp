...
namespace mau {
  ...
  VulkanQueue::VulkanQueue(VkQueue queue, VkDevice device)
  ...
  void VulkanQueue::Submit(Handle<CommandBuffer> cmd,
                           VkPipelineStageFlags  wait_stages,
                           Handle<Semaphore>     wait_semaphore,
                           Handle<Semaphore>     signal_semaphore,
    ...
    submit_info.pWaitSemaphores =
        wait_semaphore ? wait_semaphore->Ref() : nullptr;
    submit_info.pWaitDstStageMask = &wait_stages;
    submit_info.commandBufferCount = 1;
    submit_info.pCommandBuffers = cmd->Ref();
    submit_info.signalSemaphoreCount = signal_semaphore ? 1 : 0;
    submit_info.pSignalSemaphores =
        signal_semaphore ? signal_semaphore->Ref() : nullptr;
    VK_CALL(vkQueueSubmit(m_Queue, 1, &submit_info,
                          signal_fence ? signal_fence->Get() : VK_NULL_HANDLE));
  }

  void VulkanQueue::Submit(Handle<CommandBuffer> cmd) {
    Submit(cmd, 0u, nullptr, nullptr, nullptr);
  }

  void VulkanQueue::WaitIdle() { VK_CALL(vkQueueWaitIdle(m_Queue)); }

  PresentQueue::PresentQueue(VkQueue queue, VkDevice device)
      : VulkanQueue(queue, device) { }

  PresentQueue::~PresentQueue() { }

  void PresentQueue::Present(TUint32                 image_index,
