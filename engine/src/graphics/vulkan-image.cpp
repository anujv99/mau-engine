...
namespace mau {
  ...
  void TransitionImageLayout(Handle<CommandBuffer> cmd, Handle<Image> image,
    ...
    vkCmdPipelineBarrier(cmd->Get(), VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                         VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0u, 0u, nullptr,
                         0u, nullptr, 1u, &barrier);
  }

  void CopyBufferToImage(Handle<CommandBuffer> cmd, Buffer *buffer,
                         Handle<Image> image, TUint32 width, TUint32 height) {
    VkExtent3D extent = {
        .width = width,
        .height = height,
        .depth = 1u,
    ...
    vkCmdCopyBufferToImage(cmd->Get(), buffer->Get(), image->GetImage(),
                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
  }

  void UploadUsingStaging(Handle<Image> image, const void *data, TUint32 width,
                          TUint32 height, TUint32 channels) {
    ASSERT(image && data);

    TUint64 image_size = static_cast<TUint64>(width) *
                         static_cast<TUint64>(height) *
                         static_cast<TUint64>(channels);
  ...
  Image::Image(TUint32 width, TUint32 height, TUint32 depth, TUint32 mip_levels,
               TUint32 array_layers, VkImageType type,
               VkSampleCountFlagBits samples, VkFormat format,
               VkImageTiling tiling, VkImageUsageFlags usage)
      : m_Format(format), m_SampleCount(samples), m_Width(width),
        m_Height(height) {
    ...
    VK_CALL(vmaCreateImage(VulkanState::Ref().GetVulkanMemoryAllocator(),
                           &create_info, &alloc_info, &m_Image, &m_Allocation,
                           nullptr));
  }

  Image::Image(TUint32 width, TUint32 height, VkImage image, VkFormat format,
               VkSampleCountFlagBits samples)
      : m_Image(image), m_Format(format), m_SampleCount(samples),
  ...
  Image::~Image() {
    if (m_Allocation)
      vmaDestroyImage(VulkanState::Ref().GetVulkanMemoryAllocator(), m_Image,
                      m_Allocation);
  }
  ...
  ImageView::ImageView(Handle<Image> image, VkImageViewType view_type,
                       VkImageAspectFlags aspect_mask) {
    CreateImageView(image->GetImage(), image->GetFormat(), view_type,
                    aspect_mask);
  }
  ...
  void ImageView::CreateImageView(VkImage image, VkFormat format,
    ...
    VK_CALL(vkCreateImageView(VulkanState::Ref().GetDevice(), &create_info,
                              nullptr, &m_ImageView));
  }

  // framebuffer
  Framebuffer::Framebuffer(std::vector<Handle<ImageView>> image_views,
                           Handle<Renderpass> renderpass, TUint32 width,
                           TUint32 height) {
  ...
  Framebuffer::Framebuffer(Handle<ImageView> color, Handle<ImageView> depth,
                           Handle<Renderpass> renderpass, TUint32 width,
                           TUint32 height) {
    std::vector<VkImageView> image_views_handle = {color->GetImageView(),
                                                   depth->GetImageView()};
    CreateFramebuffer(image_views_handle, renderpass->Get(), width, height);
  }

  Framebuffer::~Framebuffer() {
    vkDestroyFramebuffer(VulkanState::Ref().GetDevice(), m_Framebuffer,
                         nullptr);
  }

  void Framebuffer::CreateFramebuffer(std::vector<VkImageView> image_views,
    ...
    VK_CALL(vkCreateFramebuffer(VulkanState::Ref().GetDevice(), &create_info,
  ...
  Sampler::Sampler() {
    ...
    create_info.maxAnisotropy = VulkanState::Ref()
    ...
    VK_CALL(vkCreateSampler(VulkanState::Ref().GetDevice(), &create_info,
                            nullptr, &m_Sampler));
  }
  ...
  Texture::Texture(const String &image_path) {
    ...
    if (raw_image.Data) {
      m_Image = make_handle<Image>(
          raw_image.Width, raw_image.Height, 1u, 1u, 1u, VK_IMAGE_TYPE_2D,
          VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB,
          VK_IMAGE_TILING_OPTIMAL,
          VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
      UploadUsingStaging(m_Image, raw_image.Data, raw_image.Width,
                         raw_image.Height, 4u);
      m_ImageView = make_handle<ImageView>(m_Image, VK_IMAGE_VIEW_TYPE_2D,
