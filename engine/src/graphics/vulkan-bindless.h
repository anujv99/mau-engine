...
namespace mau {
  ...
  public:
    ...
    AddAccelerationStructure(const Handle<AccelerationBuffer> &accel_struct);
    RTObjectHandle AddRTObject(const RTObjectDesc &desc);

  public:
    inline const std::vector<VkDescriptorSetLayout> &
    GetDescriptorLayout() const {
      return m_DescriptorLayouts;
    }
    inline const std::vector<VkDescriptorSet> &GetDescriptorSet() const {
      return m_DescriptorSets;
    }

  private:
    BufferHandle AddBufferInternal(const Handle<UniformBuffer> &buffer,
