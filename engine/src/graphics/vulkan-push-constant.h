...
namespace mau {
  ...
  template<typename T>
  inline PushConstant<T>::PushConstant(const T& init): PushConstantBase(sizeof(T)) {
    SetData(reinterpret_cast<const void*>(&init));
  }

  template<typename T>
  inline PushConstant<T>::~PushConstant() {}

  template<typename T>
  inline void PushConstant<T>::Update(const T& data) {
    SetData(reinterpret_cast<const void*>(&data));
  }

  template<typename T>
  inline T PushConstant<T>::GetData() const {
