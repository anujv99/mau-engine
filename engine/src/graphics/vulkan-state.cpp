...
namespace mau {
  ...
  template <typename T> T load_instance_function(VkInstance instance, std::string_view name) {
  ...
  template <typename T> T load_device_function(VkDevice device, std::string_view name) {
  ...
  void VulkanState::Init(std::string_view app_name, void *window) {
    ...
    if (m_Validation) {
      ...
      debug_messenger_create_info.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT |
  ...
  void VulkanState::SetValidationSeverity(TUint32 flags) noexcept { m_ValidationSeverity = flags; }
