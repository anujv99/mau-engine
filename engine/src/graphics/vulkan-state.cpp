...
#define LOAD_INSTANCE_FUNCTION(name)                                           \
  name = load_instance_function<PFN_##name>(m_Instance, #name)
#define LOAD_DEVICE_FUNCTION(name)                                             \
  name = load_device_function<PFN_##name>(m_Device->GetDevice(), #name)
...
namespace mau {
  ...
  VkDebugUtilsMessengerEXT create_debug_utils_messenger(
    ...
    auto                     vkCreateDebugUtilsMessengerEXT =
        load_instance_function<PFN_vkCreateDebugUtilsMessengerEXT>(
            instance, "vkCreateDebugUtilsMessengerEXT");

    if (vkCreateDebugUtilsMessengerEXT) {
      VK_CALL(vkCreateDebugUtilsMessengerEXT(instance, &create_info, nullptr,
                                             &debug_messenger));
  ...
  void destroy_debug_utils(VkInstance               instance,
                           VkDebugUtilsMessengerEXT messenger) {
    auto vkDestroyDebugUtilsMessengerEXT =
        load_instance_function<PFN_vkDestroyDebugUtilsMessengerEXT>(
  ...
  VulkanState::VulkanState(bool enable_validation)
      : m_Validation(enable_validation) {
    ...
    VK_CALL(vkEnumerateInstanceLayerProperties(
        &instance_layer_count, m_AvailableInstanceLayers.data()));

    // get available instance extensions
    uint32_t instance_extension_count = 0;
    VK_CALL(vkEnumerateInstanceExtensionProperties(
        nullptr, &instance_extension_count, nullptr));
    m_AvailableInstanceExtensions.resize(
        static_cast<size_t>(instance_extension_count));
    VK_CALL(vkEnumerateInstanceExtensionProperties(
        nullptr, &instance_extension_count,
        m_AvailableInstanceExtensions.data()));
  ...
  VulkanState::~VulkanState() {
    ...
    if (m_DebugMessenger)
      destroy_debug_utils(m_Instance, m_DebugMessenger);
  ...
  void VulkanState::Init(std::string_view app_name, void *window) {
    ...
    create_info.ppEnabledLayerNames =
        reinterpret_cast<const char *const *>(m_InstanceLayers.data());
    create_info.enabledExtensionCount =
        static_cast<uint32_t>(m_InstanceExtensions.size());
    create_info.ppEnabledExtensionNames =
        reinterpret_cast<const char *const *>(m_InstanceExtensions.data());
    ...
    if (m_Validation) {
      ...
      debug_messenger_create_info.pUserData =
          reinterpret_cast<void *>(&m_ValidationSeverity);

      create_info.pNext =
          reinterpret_cast<void *>(&debug_messenger_create_info);
    }

    // create vulkan instance
    VK_CALL_REASON(vkCreateInstance(&create_info, nullptr, &m_Instance),
    ...
    VK_CALL_REASON(
        glfwCreateWindowSurface(m_Instance, glfw_window, nullptr, &m_Surface),
    ...
    // create swapchain
    m_Swapchain = make_handle<VulkanSwapchain>(m_Device->GetDevice(),
  ...
  bool VulkanState::EnableInstanceLayer(std::string_view layer_name) noexcept {
    if (m_Instance != VK_NULL_HANDLE) {
      LOG_ERROR("cannot enable instance layer [%s]: instance already created",
                layer_name.data());
      return false;
    }
    ...
    if (!found) {
      LOG_ERROR("cannot enable instance layer [%s]: layer not found",
                layer_name.data());
      return false;
    }
  ...
  bool VulkanState::EnableInstanceExtension(
      std::string_view extension_name) noexcept {
    if (m_Instance != VK_NULL_HANDLE) {
      LOG_ERROR(
          "cannot enable instance extension [%s]: instance already created",
          extension_name.data());
      return false;
    }
    ...
    if (!found) {
      LOG_ERROR("cannot enable instance extension [%s]: extension not found",
                extension_name.data());
      return false;
    }
  ...
  void VulkanState::SetValidationSeverity(VulkanValidationLogSeverity severity,
                                          bool enabled) noexcept {
    m_ValidationSeverity = enabled ? m_ValidationSeverity | severity
                                   : m_ValidationSeverity & ~severity;
  }
  ...
  void VulkanState::PickPhysicalDevice() {
    ...
    VK_CALL(vkEnumeratePhysicalDevices(m_Instance, &physical_device_count,
                                       nullptr));
    std::vector<VkPhysicalDevice> available_physical_devices(
        static_cast<size_t>(physical_device_count));
    m_AvailablePhysicalDevices.reserve(
        static_cast<size_t>(physical_device_count));
    VK_CALL(vkEnumeratePhysicalDevices(m_Instance, &physical_device_count,
    ...
    m_SelectedPhysicalDeviceIndex =
        static_cast<TUint32>(selected_physical_device);
    m_PhysicalDevice = available_physical_devices[static_cast<size_t>(
    ...
    vkGetPhysicalDeviceProperties2(m_PhysicalDevice,
                                   &selected_physical_device_properties2);
    m_PhysicalDeviceProperties =
        selected_physical_device_properties2.properties;

    LOG_INFO("selected physical device: " LOG_COLOR_PINK "%s",
             m_AvailablePhysicalDevices[static_cast<size_t>(
                                            m_SelectedPhysicalDeviceIndex)]
                 .deviceName);
  }
  ...
  bool VulkanState::CreateCommandPool(VkQueueFlagBits queue_type) {
    ...
    if (m_CommandPools.find(queue_type) != m_CommandPools.end()) {
      LOG_WARN("command pool with queue type %s already exists",
               string_VkQueueFlagBits(queue_type));
      return false;
    }
    ...
    m_CommandPools[queue_type] =
        make_handle<CommandPool>(m_Device->GetDevice(), queue_index, flags);
    return true;
  }
  ...
  void VulkanState::InitTracy() {
    m_TracyCmdBuf =
        GetCommandPool(VK_QUEUE_GRAPHICS_BIT)->AllocateCommandBuffers(1)[0];
    m_TracyContext = TracyVkContext(m_PhysicalDevice, m_Device->GetDevice(),
                                    m_Device->GetGraphicsQueue()->Get(),
                                    m_TracyCmdBuf->Get());
  }
