...
namespace mau {
  ...
  bool LambertianPass::PostBuild(TUint32 swapchain_image_count) {
    ...
    if (!source_image || !depth_source_image)
      return false;
    ...
    for (TUint32 i = 0; i < swapchain_image_count; i++) {
      Handle<Image> img = make_handle<Image>(
          width, height, 1, 1, 1, VK_IMAGE_TYPE_2D, VK_SAMPLE_COUNT_4_BIT,
          source_image->GetImage()->GetFormat(), VK_IMAGE_TILING_OPTIMAL,
          VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
      Handle<ImageView> img_view = make_handle<ImageView>(
          img, VK_IMAGE_VIEW_TYPE_2D, VK_IMAGE_ASPECT_COLOR_BIT);
      Handle<Image> depth_img = make_handle<Image>(
          width, height, 1, 1, 1, VK_IMAGE_TYPE_2D, VK_SAMPLE_COUNT_4_BIT,
          depth_source_image->GetImage()->GetFormat(), VK_IMAGE_TILING_OPTIMAL,
          VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT);
      Handle<ImageView> depth_img_view = make_handle<ImageView>(
          depth_img, VK_IMAGE_VIEW_TYPE_2D, VK_IMAGE_ASPECT_DEPTH_BIT);
    ...
    if (m_Renderpass == nullptr) {
      ...
      LoadStoreOp op;
      m_Renderpass->AddColorAttachment(
          m_MSAAImages[0]->GetFormat(), m_MSAAImages[0]->GetSamples(), op,
          VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
      op.StoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
      m_Renderpass->SetDepthAttachment(
          m_MSAADepthImages[0]->GetFormat(), m_MSAADepthImages[0]->GetSamples(),
          op, VK_IMAGE_LAYOUT_UNDEFINED,
          VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
      op.StoreOp = VK_ATTACHMENT_STORE_OP_STORE;
      m_Renderpass->SetResolveAttachment(
          source_image->GetImage()->GetFormat(),
          source_image->GetImage()->GetSamples(), op, VK_IMAGE_LAYOUT_UNDEFINED,
    ...
    for (TUint32 i = 0; i < swapchain_image_count; i++) {
      ...
      Handle<ImageResource> this_depth_source_image =
          depth_source.GetResource(i);

      std::vector<Handle<ImageView>> image_views = {
          m_MSAAImageViews[i], m_MSAADepthImageViews[i],
          this_source_image->GetImageView()};

      Handle<Framebuffer> fbo =
          make_handle<Framebuffer>(image_views, m_Renderpass, width, height);
